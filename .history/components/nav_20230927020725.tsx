    ...
    <div className="flex w-screen py-6 px-12 items-center">
      ...
      <div className="m-auto flex gap-x-10 bg-white/10 backdrop-blur-sm py-4 px-6 rounded-full">
        {links.map((link) => {
          return (
            <Link
              className="text-white font-semi-bold text-sm hover:-translate-y-2 transition"
              href={link.href}
              key={link.name}
            >
              {link.name}
      ...
      <div className="flex text-sm items-center text-white h-max w-max py-1 px-3 border border-white rounded-xl">
        <p>Get Started</p>
      </div>
    </div>
  );
